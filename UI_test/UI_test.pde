...
public void handleFileDialog(GButton button) {
  ...
  // File input selection
   ...
   // selectInput("Wähle","test");
    ...
    if(fname == null){
      lblFile.setText("Abbruch durch Benutzer. Bitte erneut ein Fahrprofil im CSV Format wählen");
      lblFile.setLocalColorScheme(G4P.RED_SCHEME);
     } else {
      if(fname.contains(".csv") == true){  
      lblFile.setText(fname);
      title.setText("Fahrprofil gewählt: ");
      title.setTextBold();
      btnInput.dispose();
      lblFile.moveTo(btnInput.getX(),btnInput.getY(),GControlMode.CORNER);
      lblFile.setLocalColorScheme(G4P.GREEN_SCHEME);
      title.setLocalColorScheme(G4P.GREEN_SCHEME);
      } else {
        lblFile.setText("Es liegt kein passendes Dateiformat vor. Bitte eine CSV-Datei wählen");
        lblFile.setLocalColorScheme(G4P.RED_SCHEME);  
        }
      }
   }
...
public void createFileSystemGUI(int x, int y, int w) {
  ...
  title.setText("Bitte Fahrprofil (CSV-Datei) wählen", GAlign.LEFT, GAlign.MIDDLE);
  ...
  lblFile.setText("Noch kein Fahrprofil ausgewählt");

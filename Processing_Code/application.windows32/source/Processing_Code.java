...
public void writeToFile(double servo, double zeit, double gewicht, double strom, double beschleunigungX, double beschleunigungY, double beschleunigungZ) { // for sequential sending of measured data. The head of the output file is written after pressing the "Messung starten" button 
...
public long umwandelnZeit(byte array[]) {
  if ('@' == (array[4])) {
    long temp = 0;
...
public double umwandelnDouble(byte array[]) {
  if ('@' == (array[3])) {
    ...
    if ( 2 == (array[0] & 0xF0) >> 4) {
      return -1.0f * ((double)vorkomma + (double)nachkomma / 100.0f);
    }
    return (double)vorkomma + (double)nachkomma / 100.0f;
  } else return -1;// magic error number, if something doesnt work
}
...
public int map(int x, int inMin, int inMax, int outMin, int outMax) { //map a range of values to another range of values
  ...
  if (x < inMin) {  // if not in range
    ...
    return (int)(((x - inMin) * (outMax - outMin) / (inMax - inMin) + outMin)+0.5f);// 0.5 for correct rounding
  }
}
...
/*****************************************************************************/
  public void settings() {  size(560, 620); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Processing_Code" };
    if (passedArgs != null) {
